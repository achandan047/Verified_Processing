    ...
    def __init__(self, config):
        ...
        if config.get('trained_learner', False):
            self.learner.load_state_dict(torch.load(
                os.path.join(self.base_path, 'models', config['trained_learner'])
            ))
            self.classifier.load_state_dict(torch.load(
                os.path.join(self.base_path, 'models', config['trained_classifier'])
            ))
...
class SupervisedNetwork:
    ...
    def training(self, train_dataloader, val_dataloader):
        ...
        best_f1 = 0
        patience = 0
        model_path = os.path.join(self.base_path, 'models', 'TargetModel{}.h5'.format(self.stamp))
        classifier_path = os.path.join(self.base_path, 'models', 'TargetClassifier-{}.h5'.format(self.stamp))
